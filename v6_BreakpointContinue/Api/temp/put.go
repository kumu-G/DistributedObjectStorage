...
func put(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if current == stream.Size {
			...
			// fmt.Println("Escape(hash):", url.PathEscape(hash))
			...
			fmt.Println("temp.pot.go :", hash) ////////////////////////////////
			if locate.Exist(url.PathEscape(hash)) {
				stream.Commit(false)
			} else {
				stream.Commit(true)
			}
			fmt.Println("temp.put.go Addversion stream.Hash:", stream.Hash) //test6
	...
	/*
		...
		for {
			...
			if current == stream.Size {
				...
				// url.PathEscape(hash) != stream.Hash
				// hahs 未转义
				// stream.Hash
				if hash != stream.Hash {
					stream.Commit(false)
